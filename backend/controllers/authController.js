 ...
 * Returns a 12-character password with upper, lower, digit, and special character.
 */
...
async function register(req, res) {
    ...
    try {
        ...
        // Security: Input validation - name and phone are required; password is auto-generated if omitted
        if (!name || !phone) {
            const response = { success: false, message: 'Name and phone number are required.' };
            console.log('❌ [REGISTER] Validation failed: Missing name or phone', { name, phone });
        ...
        const [existingPhone] = await db.pool.execute('SELECT id FROM users WHERE phone_number = ?', [cleanPhone]);
        ...
        // Security: Validate password strength (whether user-provided or auto-generated)
        ...
        if (!passwordValidation.valid) {
            ...
            console.log('❌ [REGISTER] Validation failed: Password invalid', { plainPassword });
            console.log('🟠 [REGISTER] Responding with:', JSON.stringify(response, null, 2));
            return res.status(400).json(response);
        }
        ...
        console.log('🟢 [REGISTER] Responding with:', JSON.stringify({ ...response, generatedPassword: response.generatedPassword ? '***' : undefined }, null, 2));
...
/**
 * Forgot Password - handles email reset link and phone OTP
 * - POST { email } → send reset link via email
 * - POST { phone } → send 6-digit OTP via SMS
 * - POST { phone, code, password } → verify OTP and reset password
 */
    ...
    try {
        ...
        if (phone && code && password) {
            ...
            const [rows] = await db.pool.query(
                'SELECT id, email, reset_code, reset_code_expiry FROM users WHERE phone_number = ? OR phone = ? LIMIT 1',
                [phone, phone]
            );
            const user = rows && rows[0];

            if (!user || !user.reset_code || user.reset_code !== code || new Date(user.reset_code_expiry) < new Date()) {
            ...
            if (user.email) clearLoginAttempts(user.email);
            return res.status(200).json({ success: true, message: 'Password reset successfully. You can now log in.' });
        }

        // ── Phone OTP Request (step 1) ────────────────────────────────────────
        if (phone && !email) {
            const user = await User.findOne({ phone });
            if (!user) {
                // Security: don't reveal whether phone exists
                return res.status(200).json({ success: true, message: 'If that phone number exists, a reset code has been sent.' });
            }
            ...
            try {
                await sendSMS(phone, `Your PledgedHub password reset code is: ${otp}. It expires in 10 minutes.`);
            } catch (smsErr) {
        ...
        try {
            await sendEmail({
                to: user.email,
                subject: 'PledgedHub Password Reset',
                html: `
                    <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
                        <h2 style="color:#2563eb">Reset Your Password</h2>
                        <p>Hi ${user.name || user.username || 'there'},</p>
                        <p>You requested a password reset for your PledgedHub account. Click the button below to set a new password:</p>
                        <p style="text-align:center;margin:32px 0">
                            <a href="${resetUrl}" style="background:#2563eb;color:white;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block">Reset Password</a>
                        </p>
                        <p style="color:#64748b;font-size:0.875rem">Or copy this link into your browser:<br>${resetUrl}</p>
                        <p style="color:#64748b;font-size:0.875rem">This link expires in <strong>1 hour</strong>. If you didn't request this, you can safely ignore this email.</p>
                        <p style="color:#64748b">— The PledgedHub Team</p>
                    </div>
                `,
                text: `Reset your PledgedHub password: ${resetUrl}\n\nThis link expires in 1 hour.`,
            });
        } catch (emailErr) {
...
async function resetPassword(req, res) {
    try {
        ...
        // Update password and clear reset fields (use null not undefined so DB actually clears them)
        ...
        console.log(`[SECURITY] Password reset successful for user: ${user.id}`);
