...
async function runMigration() {
    ...
    try {
        ...
        const [columns] = await db.execute('DESCRIBE pledges');
        ...
        const [donorUpdated] = await db.execute(`
            UPDATE pledges 
            SET donor_name = name
            WHERE donor_name IS NULL OR donor_name = ''
        `);
        console.log(`   ✅ Updated ${donorUpdated.affectedRows} pledges with donor names`);
        
        // Set purpose from description where empty
        const [purposeUpdated] = await db.execute(`
            UPDATE pledges 
            SET purpose = COALESCE(description, 'General Pledge')
            WHERE purpose IS NULL OR purpose = ''
        `);
        console.log(`   ✅ Updated ${purposeUpdated.affectedRows} pledges with purposes`);
